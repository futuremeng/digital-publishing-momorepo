 ...
 * @LastEditTime: 2025-06-23 13:42:51
 * @FilePath: /one-monorepo/apps/special-scenes/dictionary/src/router/router-guards.ts
 * @Description:
...
export function createRouterGuards(router: Router) {
  router.beforeEach(async (to, from, next) => {
    ...
    if (!token) {
      // redirect login page
      // next(LOGIN_PATH)
      navigateToApp("user-center", "/login", {
        redirect: encodeURIComponent(window.location.origin + import.meta.env.VITE_PUBLIC_PATH + to.fullPath),
      });
      return;
    }
